...
// Draw the filled arc before the needle so its fill cannot cover the indicator.
const GAUGE = '<path d="M3.34 19a10 10 0 1 1 17.32 0"/><path d="M12 14 L16 10" fill="none"/>';
...
export class MachineNodeView {
  ...
  private build(
    ...
    const symbol = footer?.kind === "storage" ? STORAGE : footer?.kind === "fluid" ? FLUID : ZAP;
    const powerColors =
      footer?.kind === "generation" ? this.palette.generation : footer ? null : this.palette.power;
    this.symbol(
      symbol,
      12,
      footerY + 8,
      powerColors?.stroke ?? this.palette.footer,
      powerColors?.fill ?? "none",
    );
    const powerLabel =
