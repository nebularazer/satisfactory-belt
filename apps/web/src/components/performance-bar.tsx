...
export function PerformanceBar({ monitor }: { monitor: RenderPerformance }) {
  ...
  return (
    <aside
      ...
      className="absolute bottom-[calc(max(1rem,env(safe-area-inset-bottom))+3rem)] left-1/2 flex h-8 -translate-x-1/2 items-center gap-3 whitespace-nowrap rounded-lg border bg-white px-3 text-xs text-muted-foreground shadow-sm tabular-nums lg:bottom-[max(1rem,env(safe-area-inset-bottom))]"
    >
