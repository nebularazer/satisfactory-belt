...
export type ProductionReplacementPreview = {
  oldNodeCount: number;
  newNodeCount: number;
  retainedConnections: number;
  disconnectedConnections: readonly string[];
  apply: () => void;
};

type Props = {
  initialSettings?: AutoBuildSettings;
  sectionName?: string;
  ...
  ) => Promise<void | ProductionReplacementPreview>;
};
...
export function AutoBuildDialog({
  ...
  const [preview, setPreview] = useState<ProductionReplacementPreview | null>(
    null,
  );
  const [outputs, setOutputs] = useState(
    initialSettings?.outputs.map((o) => ({
      itemId: o.itemId,
      rate: String(o.ratePerMinute),
    })) ?? [{ itemId, rate: "10" }],
  );
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {sectionName ? "Edit production request" : "Auto-build production"}
          </DialogTitle>
          <DialogDescription>
            {preview
              ? "Manual edits inside this section will be replaced. Other sections and saved Detailed versions stay unchanged."
              : sectionName
                ? `Editing ${sectionName}. This replaces its ${sectionNodeCount} remaining nodes and their internal links, including manual edits. Other sections and saved Detailed versions stay unchanged. Review the replacement before applying it.`
                : "Choose your outputs. We’ll build and arrange a Basic production plan with machine clocks at 100% or less, ready to edit or expand into Detailed."}
          </DialogDescription>
        </DialogHeader>
        {preview ? (
          <div className="grid gap-4">
            <p className="font-medium">Replace {sectionName}</p>
            <div className="grid gap-2 rounded-lg border border-border p-3">
              <p className="text-muted-foreground">New production targets</p>
              {outputs.map((output) => (
                <div key={output.itemId} className="flex items-center gap-2">
                  <ItemImage itemId={output.itemId} />
                  <span className="min-w-0 flex-1">
                    {findDescriptor(output.itemId)?.name}
                  </span>
                  <span>
                    {output.rate}{" "}
                    {findDescriptor(output.itemId)?.form === "solid"
                      ? "items/min"
                      : "m³/min"}
                  </span>
                </div>
            ...
            <p>
              {preview.oldNodeCount} nodes replaced by {preview.newNodeCount}{" "}
              nodes.
            </p>
            <p>
              External connections retained: {preview.retainedConnections}.
              Their flow may change with the new production rates.
            </p>
            {preview.disconnectedConnections.length > 0 && (
              <div
                role="alert"
                className="rounded-md border border-destructive p-3"
              >
                <p className="font-medium">
                  External connections to remove (
                  {preview.disconnectedConnections.length}): their original
                  machine ports cannot be matched.
                </p>
                <ul className="mt-2 list-disc pl-4">
                  {preview.disconnectedConnections.map((name, i) => (
                    <li key={i}>{name}</li>
                  ))}
                </ul>
              </div>
            ...
            <p className="text-muted-foreground">
              Undo restores the previous nodes, connections, and production
              request together.
            </p>
            {error && (
            ...
            <DialogFooter>
              ...
              >
                {preview.disconnectedConnections.length
                  ? `Replace and disconnect (${preview.disconnectedConnections.length})`
                  : "Replace section"}
              </Button>
            </DialogFooter>
          </div>
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-[0.625rem] text-muted-foreground">
                            {recipe.inputs
                              .map(
                                (input) => findDescriptor(input.itemId)?.name,
                              )
                              .join(" + ")}
                          </div>
                        </div>
