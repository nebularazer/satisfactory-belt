 ...
 * Boundary ports keep the local and factory routes joined without hiding nodes.
 */
async function logisticsBlock(
  ...
  const boundary = (
    ...
    id: key("boundary", end),
    width: 0,
    height: Math.max(1, edges.length * 48),
    layoutOptions: {
      "elk.portConstraints": "FIXED_POS",
      "elk.layered.layering.layerConstraint": end === "from" ? "FIRST" : "LAST",
    },
    ports: edges.map((link, index) =>
      port(
  ...
  const graph = await elk.layout<ElkNode>({
    ...
    children: [
      ...nodes
        .filter((node) => !structure.returnNodes.has(node.configuration.id))
        .map(nodeGraph),
      ...(inputs.length ? [boundary(inputs, "from")] : []),
      ...(outputs.length ? [boundary(outputs, "to")] : []),
    ],
    edges: links
      .filter(
        (link) =>
  ...
  for (const [edges, end] of [
    ...
    for (const link of edges) {
      ...
      if (!route) throw new Error("Could not route a logistics boundary.");
      const entering = end === "from";
      const point = {
        x: entering ? 0 : width,
        y: (entering ? route[0]! : route.at(-1)!).y,
      };
      block.ports.push(
        port(
      ...
      block.tails.set(
        link.id,
        entering ? [point, ...route] : [...route, point],
      );
      routes.delete(link.id);
    }
  }
...
export async function arrangeCanvas(
  ...
  for (const group of productionStructure(sorted).logistics)
    blocks.push(await logisticsBlock(sorted, group, elk));
